<template>
  <v-container fluid>
    ...
    <!-- Table -->
    <v-data-table :items="rows" :loading="pending" :items-per-page="pageSize" :page.sync="page"
      :server-items-length="total" item-value="code" class="elevation-1">
      <template #headers>
        <tr>
    ...
    <!-- Dialog -->
    <language-dialog v-model="showDialog" :editItem="editItem" title="Language" @refresh="refresh" resource="language"/>
  </v-container>
</template>
...
const {
  ...
  exportExcel,
  deleteItem
} = useMasterTable<Language>('language')

const showDialog = ref(false)
const editItem = ref<Language | null>(null)

function edit(item: Language) {
  editItem.value = item
